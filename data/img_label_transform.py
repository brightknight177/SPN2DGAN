...
class ExtPlusResize(object):
    ...
    def __call__(self, imgA, imgB, lbl):
        ...
        """
        return F.resize(imgA, self.size, self.interpolation), F.resize(imgB, self.size, self.interpolation), F.resize(lbl, self.size, Image.NEAREST)
...
class ExtPlusRandomCrop(object):
    ...
    def __call__(self, imgA, imgB, lbl):
        ...
        """
        assert imgA.size == lbl.size, 'size of img and lbl should be the same. %s, %s' % (imgA.size, lbl.size)
        if self.padding > 0:
            imgA = F.pad(imgA, self.padding)
            imgB = F.pad(imgB, self.padding)
            lbl = F.pad(lbl, self.padding)

        # pad the width if needed
        if self.pad_if_needed and imgA.size[0] < self.size[1]:
            imgA = F.pad(imgA, padding=int((1 + self.size[1] - imgA.size[0]) / 2))
            imgB = F.pad(imgB, padding=int((1 + self.size[1] - imgB.size[0]) / 2))
            lbl = F.pad(lbl, padding=int((1 + self.size[1] - lbl.size[0]) / 2))

        # pad the height if needed
        if self.pad_if_needed and imgA.size[1] < self.size[0]:
            imgA = F.pad(imgA, padding=int((1 + self.size[0] - imgA.size[1]) / 2))
            imgB = F.pad(imgB, padding=int((1 + self.size[0] - imgB.size[1]) / 2))
            lbl = F.pad(lbl, padding=int((1 + self.size[0] - lbl.size[1]) / 2))

        i, j, h, w = self.get_params(imgA, self.size)

        return F.crop(imgA, i, j, h, w), F.crop(imgB, i, j, h, w), F.crop(lbl, i, j, h, w)

    def __repr__(self):
        return self.__class__.__name__ + '(size={0}, padding={1})'.format(self.size, self.padding)


class ExtPlusRandomHorizontalFlip(object):
